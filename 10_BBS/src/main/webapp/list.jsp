...
<% 
	...
	// 4-1. 끝페이지(endPage)가 전체페이지 수(totalPage) 보다 크면 끝페이지를 전체페이지로 변경처리
		...
		<c:choose>
			...
			<c:otherwise>
				<c:forEach var="vo" items="${list }">
					<tr>
						<td>${vo.b_idx }</td>
						<td class="align-left"><a href="view.jsp?b_idx=${vo.b_idx }&cPage=${pvo.nowPage}">${vo.subject}</a></td>
						<td>${vo.writer }</td>
